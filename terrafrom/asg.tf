...
resource "aws_appautoscaling_target" "ecs_asg" {
  ...
  depends_on = [aws_ecs_service.ecs_service] #will create ecs before auto scaling group
}
...
resource "aws_appautoscaling_policy" "ecs_policy" {
  ...
  resource_id        = "service/service/${var.project_name}-${var.environment}-cluster/${var.project_name}-${var.environment}-service"
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace  = "ecs"
